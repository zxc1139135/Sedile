...
def gen_Harmonic_coeffs(alpha_s, beta_s, p, is_K1=0):
    if is_K1 == 1:
        num_alpha = 1
    else:
...
def gen_Harmonic_lambda_s(alpha_s, p):
...
def Harmonic_decoding(f_eval, worker_idx, p):
    # f_eval     : [RT X d ]
    # worker_idx : [ 1 X RT]
    # output     : [ 1 X d ]
    ...
    t1 = time.time()
    # print(alpha_s_eval)
    lambda_s = gen_Harmonic_lambda_s(alpha_s_eval, p).astype('int64')
    t2 = time.time()
    # print(lambda_s.shape)
    f_recon = np.mod(np.dot(lambda_s,f_eval), p)
    t3 = time.time()
    print ('time info for HMC_dec', t1-t0, t2-t1, t3-t2)
    return f_recon


def HMC_encoding(X, N, K, T, p):
    m = len(X)
    d = len(X[0])
    ...
    U = gen_Harmonic_coeffs(alpha_s, beta_s, p)
...
def HMC_encoding_w_Random(X, R_, N, K, T, p):
    m = len(X)
    d = len(X[0])
    X_sub = np.zeros((K+T, m//K, d), dtype='int64')
    # print(X_sub.shape)
...
def HMC_encoding_W_Random_partial(X, R_, N, K, T, p, worker_idx):
    m = len(X)
    d = len(X[0])
    # print(m,d,m//K)
    X_sub = np.zeros((K+T, m//K, d),dtype='int64')
    ...
    U = gen_Harmonic_coeffs(alpha_s_eval, beta_s, p)
...
def HMC_decoding(f_eval, N, K, T, worker_idx, p):
    ...
    U_dec = gen_Harmonic_coeffs(beta_s, alpha_s_eval, p)
...
# secure matrix multiplication (single reshare for degree reduction)
def matrix_multiply(X1, X2, p, T, dot_ON):

    t0_matmul = time.time()
    # multiply the current secret shares
    if dot_ON:
        # multiply the current secret shares
        mult = np.mod(X1.dot(X2), p)
    else:
        # multiply the current secret shares
        mult = np.mod(np.matmul(X1,X2), p)
    ...
    # store secret shares
    Q = np.empty((2*T+1, row, col), dtype='int64')

    # workers 1 to 2T+1 sends shares
    for j in range(1, 2*T+2):

        if rank == j:
            # XXX create secret shares for all workers
            Qss = Harmonic_encoding(mult,N,T,p)
            # own secret share
            Q[j-1, :, :] = Qss[j-1, :, :]

            # secret share q
            for j in range(1, rank) +  range(rank+1,N+1):
            ...
            Q[j-1,:,:] = np.reshape(data,(row,col))
    # XXX need a vector of lagrange coeffs for 1 to N (in fact only 1 to 2T+1 is enough) but it doesn't matter much,
    # also can take this out of the function and compute it once and store to speed up later
    Lcoeffs = gen_Harmonic_lambda_s(range(1, 2*T+2), p)

    out = np.empty((row, col), dtype='int64')

    for j in range(2*T+1):
        # index 0 to 2T+1, XXX MAKE THIS FINITE FIELD OPERATION WITH MOD
        out += Lcoeffs[0][j] * Q[j,:,:]
...
def MultPassive(A_SS_T, B_SS_T, R_SS_T, R_SS_2T, N,T,p):
    ...
    delta = Harmonic_decoding(delta_SS_2T[worker_idx,:],worker_idx,p)
...
def TruncPr(a_BGW, k, m, p, N, T):
    ...
    RT_BGW = T+1
    worker_idx = random.sample(range(N),RT_BGW)

    c = Harmonic_decoding(c_BGW[worker_idx,0,:],worker_idx,p)
